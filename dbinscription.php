...
if (isset($_POST["submit"])) { //On vérifie que l'utilisateur a bien rempli le champs "mail".
    ...
    $verif = "SELECT * FROM stagiaire WHERE Email = '$pseudoi'"; // Mise en forme de la requête que l'on va envoyer, on récolte la ligne qui associe le contenu du champs email au contenu entré par l'utilisateur.
  	...
  	$carteid = $_POST["carteid"]; //je récupère la valeur de l'input carte identité
  	$photoid = $_POST["photoid"]; //je récupère la valeur de l'input photo
    ...
    if(mysqli_num_rows($result2) > 0) { // Si la base de donnée nous retourne au moins un élément (ce qui signifie que le pseudo existe déjà), alors exécuter la suite :
    	...
    	$sql = "INSERT INTO stagiaire(`nom`, `prenom`, `date de naissance`, `adresse`, `code_postal`, `Ville`, `num_inscription_JS`, `num_securite_social`, `tel`, `Email`, `sexe`, `statut`, `mdp`, `membreAssoFSCF`, `numeroLicence`, `membreStructurePartenaire`, `priseEnChargeFinanciere`, `organisme`, `attestationInscription_idattestationInscription`, `personne_urgence`, `tel_urgence`, `medical`) VALUES ('$name','$firstname','$date','$adress','$codepo','$city','$numjs','$secu','$tel','$pseudoi','$sex','','','','$licence','$aide','$montaide','$orgaide',1,'$contacturg','$telurg','$alleregi')";
